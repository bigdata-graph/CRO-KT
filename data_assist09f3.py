...
class DataProcess():
    ...
    def generate_user_sequence(self, seq_file):
        ...
        for ui in ui_df:
            ...
            for q in Q:
                for qa in QA:
                    if qa[1]!=q[1]:
                        if qa[1]==1 and abs(qa[2]-q[2])>=0.7 and qa[2]>q[2] and qa[3]==q[3]:
                            tmp_ans[QA.index(qa)]=0
                            QA[QA.index(qa)][1]=0
                            print('偶发')
                        elif qa[1]==0 and abs(qa[2]-q[2])>=0.7 and qa[2]<q[2] and qa[3]==q[3]:
    ...
    def generate_user_sequence2(self):
        ...
        for ui in range(0, len(all_user)):
            user = all_user[ui]
            tmp_problems = user[2]
            tmp_skills = user[1]
            tmp_ans = user[3]
                ...
                for qa in QA:
                    ...
                    for qh in qH:
                        if qa[3] == qh[3] and abs(qa[2] - qh[2]) < 0.05:
                            d = abs(qH.index(qh) - QA.index(qa)) + 1
                            sumc += (qh[2] * qh[1]) / d
                            sumd += qh[2] / d
                        else:
                            continue
    ...
    def read_user_sequence3(self, filename, max_len=200, min_len=3, shuffle_flag=True):
        ...
        np.savez(os.path.join(self.data_folder, "%s.npz" % 'assist09f3'), problem=problem, y=y, skill=skill,
                 real_len=real_len, skill_num=skill_num, problem_num=pro_num)
...
if __name__ == '__main__':
    ...
    # DP.pro_skill_graph()
    # DP.pro_skill_graph1()
    # DP.generate_user_sequence('dataf1.txt')
    # DP.generate_user_sequence1('data2.txt')
    # DP.generate_user_sequence2('dataf3.txt')
