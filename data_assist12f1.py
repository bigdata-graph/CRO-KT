...
#处理数据
class DataProcess():
    def __init__(self, data_folder='assist12', file_name='2012-2013-data-with-predictions-4-final.csv', min_inter_num=3):
    ...
    def process_csv(self):
        ...
        df = pd.read_csv(data_path, low_memory=False, encoding="ISO-8859-1",nrows=7123270)
    ...
    def pro_skill_graph(self):
        ...
        for pro_id in range(0,47104):
            ...
            tmp_skills = tmp_df_0['skill_id']
            print(tmp_skills)
            if tmp_skills not in skill_id_dict:
                skill_id_dict[tmp_skills] = skill_cnt
                skill_cnt += 1
    ...
    def generate_user_sequence1(self, seq_file):
        ...
        for ui in tqdm(ui_df):
            ...
            tmp_skills = list(tmp_inter['skill_id'].astype('int'))
            tmp_ans = list(tmp_inter['correct'].astype('int'))
            ...
            for qi, qc, qk in zip(tmp_problems, tmp_ans, tmp_skills):
                ...
                q.append(qc)
                q.append(qd)
                q.append(qk)  # 使用参数作为分割线将原始字符串并组织成列表
                Q.append(q)
            # 以下伪代码将融入观点
            QA = []
            for q in Q:
    ...
    def generate_user_sequence2(self, seq_file):
        # generate user interaction sequence
        # and write to data.txt
        ...
        for ui in tqdm(ui_df):
            ...
            for q in Q:
                ...
                for qa in QA:
                    ...
                    sumd = 0
                    for qh in qH:
                        if qa[3] == qh[3] and abs(qa[2] - qh[2]) < 0.1:
            ...
            user_inters.append([[len(tmp_skills)], tmp_skills, tmp_problems, tmp_ans])
        pro_skill_adj = []
        skill_id_dict, skill_cnt = {}, 0
        # for pro_id in range(len(problems)):
        #     tmp_df = df[df['problem_id'] == problems[pro_id]]
    ...
    def read_user_sequence(self, filename, max_len=200, min_len=3, shuffle_flag=True):
        ...
        while index < len(lines):
            ...
            tmp_skills = tmp_skills    # for assist09
        ...
        np.savez(os.path.join("assist12", "assist12.npz"), problem=problem, y=y, skill=skill, real_len=real_len, skill_num=skill_num, problem_num=pro_num)
    ...
    def read_user_sequence1(self, filename, max_len=200, min_len=3, shuffle_flag=True):
        ...
        np.savez(os.path.join("assist12", "assist121.npz"), problem=problem, y=y, skill=skill,
    ...
    def read_user_sequence2(self, filename, max_len=200, min_len=3, shuffle_flag=True):
        ...
        np.savez(os.path.join("assist12", "assist122.npz"), problem=problem, y=y, skill=skill,
                 real_len=real_len, skill_num=skill_num, problem_num=pro_num)
    ...
    def read_user_sequence3(self, filename, max_len=200, min_len=3, shuffle_flag=True):
        ...
        np.savez(os.path.join("assist12", "assist12f1.npz"), problem=problem, y=y, skill=skill,
                 real_len=real_len, skill_num=skill_num, problem_num=pro_num)
...
if __name__ == '__main__':
    ...
    # DP.process_csv()
    # DP.pro_skill_graph()
    DP.generate_user_sequence1('dataf1.txt')
    ...
    # DP.generate_user_sequence2('data2.txt')
    # DP.read_user_sequence2(os.path.join(data_folder, 'data2.txt'))
    DP.read_user_sequence3(os.path.join(data_folder, 'dataf1.txt'))
